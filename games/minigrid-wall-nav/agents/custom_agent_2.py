"""
CustomAgent2 - FAILED EXPERIMENT (Wandering Agent)
==================================================

ISSUES IDENTIFIED (Training run: 5000 episodes):
------------------------------------------------
1. AGENT NEVER REACHED THE GOAL
   - Almost all episodes hit max_steps=200 (timeout)
   - Reward ~12-14 came from "move forward" bonus, not goal
   
2. REWARD SHAPING WAS TOO STRONG
   - +0.05 per forward move overwhelmed the +1 goal reward
   - Agent learned: "just wander forever collecting small bonuses"
   - No incentive to actually finish the task
   
3. STATE SPACE EXPLODED (81,000 states)
   - Too many unique states with direction + distances
   - Agent memorized instead of generalizing
   
4. NO PENALTY FOR FAILURE
   - Timeout gave no negative signal
   - Wandering was actually optimal behavior!

LESSONS LEARNED:
----------------
- Reward shaping must be SMALL relative to task reward
- Need step penalty to encourage efficiency
- Need timeout penalty to discourage wandering
- State space should be as small as possible

SEE: CustomAgent3 for fixes applied
"""
...
class CustomAgent2:
    """
    Tabular Q-Learning Agent with Direction and Distance Features.
    
    NOTE: This version had reward shaping issues - see module docstring above.
