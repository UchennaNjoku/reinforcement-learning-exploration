"""
CustomAgent4 - WALL COLLISION FIX
=================================

FIXES APPLIED (based on CustomAgent3 failures):
------------------------------------------------
1. ADDED WALL COLLISION PENALTY (-0.5)
   Problem: Agent got stuck spinning in place forever
   - Facing wall → turn → facing wall → turn → ... (infinite loop)
   - Agent preferred spinning (-0.09/step) over risking timeout (-1.0)
   
   Solution: Detect when move_forward doesn't change position
   - If action == 2 (move_forward) and pos doesn't change = hit wall
   - Add -0.5 penalty to teach "don't walk into walls"
   
2. PREVIOUS FIXES (from v3):
   - Step penalty: -0.08/step
   - Forward bonus: +0.02
   - Turn penalty: -0.01
   - Timeout penalty: -1.0
...
| Wall bump (fwd but stuck)  | -0.50  |
| Reach goal                 | +1.0   |
| Timeout (max_steps)        | -1.0   |

EXPECTED BEHAVIOR:
------------------
- Agent should learn to turn when facing a wall
- No more infinite spin cycles
- Better exploration of the environment

SEE: Training loop in train.py for wall collision detection logic
"""
...
class CustomAgent4:
    """
    Tabular Q-Learning Agent with wall collision awareness.
