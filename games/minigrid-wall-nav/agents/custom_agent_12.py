"""
CustomAgent12 - Tabular Q-Learning with Curriculum Learning Support
====================================================================

This agent is designed to work with curriculum learning in the training loop.
The curriculum progressively increases environment difficulty:

- Phase 1 (0-10% episodes): No wall (wall_length=0)
- Phase 2 (10-30% episodes): Small wall (wall_length=2)  
- Phase 3 (30%+ episodes): Full wall (wall_length=4)

The agent uses the SAME state representation as CustomAgent11:
(dir, dxg, dyg, dxw, dyw, front_blocked, left_blocked, right_blocked)

Knowledge transfers between phases because:
- Phase 1 learns "move toward goal" (clean navigation policy)
- Phase 2 learns "detour around small obstruction" (obstacle avoidance)
- Phase 3 scales that behavior to full difficulty

No Q-table reset between phases - continuous learning.
"""
...
class CustomAgent12:
    """
    Tabular Q-Learning agent with curriculum learning support.
    
    Identical architecture to CustomAgent11, but designed to be trained
    with progressive difficulty curriculum.
    """
    ...
    def _get_best_action(self, state: Tuple) -> int:
        """
        Get best action with tie-breaking preference.
        ...
        This prevents spinning behavior when "turn left" and "turn right"
        look equally good.
        """
